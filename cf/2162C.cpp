...
void solve(int tc)
{
    ...
    int afirstSetBitPosition = -1;

    // Loop from the highest index down to the lowest
    for (int i = a1.size() - 1; i >= 0; --i)
    {
        if (a1[i])
        {
            afirstSetBitPosition = i;
            break;
        }
    }
    int bfirstSetBitPosition = -1;
    ...
    for (int i = b1.size() - 1; i >= 0; --i)
    {
        if (b1[i])
        {
            bfirstSetBitPosition = i;
            break;
        }
    }
    if(bfirstSetBitPosition>afirstSetBitPosition){
        cout<<-1<<'\n';
        return;
    }

    vector<int> ans;
    for (int i = 0; i < (int)zerotoone.size(); i++)
    {
        uint32_t num = 0;
        int index_to_set = zerotoone[i];
        num = num | (1U << index_to_set);
        a=a^num;
        dbg(a,num);
        ans.pb(num);
    }
    for (int i =  (int)onetozero.size()-1; i>=0 ; i--)
    {
        uint32_t num = 0;
        int index_to_set = onetozero[i];
        num = num | (1U << index_to_set);
        a=a^num;
        dbg(a,num);
        ans.pb(num);
