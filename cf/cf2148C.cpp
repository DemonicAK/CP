...
void solve(int tc)
{
    ...
    while(n){
        ...
        int distparity = (time-lasttime)&1;

        int maxscore = time-lasttime;


        if(distparity){
            if(flip)currscore+=maxscore;
            else currscore+=maxscore-1;
        }
        else {
            if(flip)currscore+=maxscore-1;
            else currscore+=maxscore;
        }
    ...
    if(lasttime!=m)currscore+=(m-lasttime);
    cout<<currscore<<'\n';
