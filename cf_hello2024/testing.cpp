...
{
    if (rc == qc || rr == qr)
     { //cout<<rr<<rc<<br<<bc<<qr<<qc;
    if((rr<br && br<qr) || (qr<br && br<rr) || (rc<bc && bc<qc) || (qc<bc && bc<rc))
        return 2;
       return 1;}

    if (((br + bc) % 2 == 0 && (qr + qc) % 2 == 0) || ((br + bc) % 2 == 1 && (qr + qc) % 2 == 1))
    {
        ...
        if (rdiagq == rdiagb && rdiagr == rdiagb)
        {
            if ((qc < rc && rc < bc) || (bc < rc && rc < qc))
                return 2;
            else
                return 1;
        }
        if (ldiagq == ldiagb && ldiagr == ldiagb)
        {
            if ((qc < rc && rc < bc) || (bc < rc && rc < qc))
                return 2;
            else
...
void solve(){
    // int n; see(n);
    int rr,rc,br,bc,qr,qc;
    see(rr);see(rc);see(br);see(bc);see(qr);see(qc);
    cout<<minMovesToCaptureTheQueen(rr,rc,br,bc,qr,qc)<<endl;
