...
//----------SOLUTION----------
bool inbetween(int chi, int tak, int oth, vector<vector<int>> &points)
{
    int rmin = min(points[chi][0], points[tak][0]);
    int rmax = max(points[chi][0], points[tak][0]);

    int cmin = min(points[chi][1], points[tak][1]);
    int cmax = max(points[chi][1], points[tak][1]);
dbg(rmin, rmax, cmin, cmax);
    if (rmin <= points[oth][0] && points[oth][0] <= rmax)
    {
        if (cmin <= points[oth][1] && points[oth][1] <= cmax)
        {
            return false;
        }
    }
...
int numberOfPairs(vector<vector<int>> &points)
{
    ...
    for (int c = 0; c < points.size(); c++)
    {
        ...
        {
            // chisato col < takina col
            if (c!= t && points[t][1] <= points[c][1] && points[c][0] <= points[t][0])
            {
            // dbg(points[c][1], points[t][1],c,t);
                for (int k = 0; k < points.size(); k++)
                {
                    if (k != c && k != t)
                    {
            dbg(points[c][1], points[t][1],c,t);
                        if (inbetween(c, t, k, points))cnt++;
                        dbg(inbetween(c, t, k, points)); 
...
void solve()
{
    DEBUG;
    // see(n);
    vector<vector<int>> points {{1,1},{1,3},{3,1}};
