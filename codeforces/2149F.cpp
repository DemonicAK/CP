...
int solver(int h, int d, int last,vvi&dp)
{
    if (d == 0)
        return 1;
    if (h > (int)(d * (d + 1) / 2)+last*d)
        return d;
    if(dp[h][d]!=-1)return dp[h][d];

    int c1 = 1e18;
    if (h - last - 1 > 0)
        c1 = solver(h - last - 1, d - 1, last + 1,dp)+1;
    int c2 = solver(h + 1, d, 0,dp)+1;
    // dbg(last,c1,c2);
    return dp[h][d]=min(c1, c2);
}
//----------SOLUTION----------
void solve(int tc)
{
