...
seev(b,m);
    sortall(b);
    a.insert(a.begin(), -infL);
    n++; //imp....bcoz we inserted a[0] = -infL
    forn(i,1,n)
    {
        auto it = lower_bound(all(b), -15, [&](int x, int _)
                              {
            assert(_ == -15);
            return x-a[i] < a[i-1]; });
        if (it == b.end())
        ...
        if (a[i] < a[i - 1] && j - a[i] < a[i - 1])
            continue; // it tells it is not possible to get  it sorted
        a[i] = min((a[i] < a[i - 1] ? infL : a[i]), 
        (j - a[i] < a[i - 1] ? infL : j - a[i])); //for ind=0
    }
