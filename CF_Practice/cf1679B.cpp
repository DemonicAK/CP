...
void solve(int t)
{
    ...
    vi lastFirstQuery(n, -1);
    vi lastFirstQueryValue(n, 0);
    rep(i, n) lastFirstQueryValue[i] = v[i];
    ...
    tc(q)
    {
        ...
        if (optype == 1)
        {
            int i, x;
            see(i, x);
            --i; // To convert to 0-based index
            // int current_value = (lastSecondQuery > lastFirstQuery[i] ? lastSecondQueryValue : lastFirstQueryValue[i]);
            int current_value = 0;
            if (lastSecondQuery > lastFirstQuery[i])
                current_value = lastSecondQueryValue;
            else
                current_value = lastFirstQueryValue[i];

            currsum = currsum - current_value + x;
            lastFirstQuery[i] = query_count;
        ...
        else
        {
            int x;
            see(x);
            currsum = static_cast<ll>(n) * x;
            lastSecondQuery = query_count;
